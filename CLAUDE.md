...
- **While (a) denies writes:** prepare app and protocol implementation locally
  as worktree branches and patch files under `deliverables/`, then hand them to
  ...
  to them again until the write gate clears.
...
- `state/loop.json`: local completion state
