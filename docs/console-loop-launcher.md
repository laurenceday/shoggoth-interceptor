...
session with the loop prompt, logs land in `state/loops/`, GET `/api/loops`
...
- The console fiat run's demo evidence layout in `deliverables/console-demo/`.
...
1. **Spawn `claude -p` detached from the console process.** Chosen. One
   ...
   `state/loops/`, a pidfile beside it. Trade named: the console process
...
- **Secrets.** The spawned session inherits the operator's environment by
  ...
  state/. Log filenames are timestamped, content is the session transcript.
...
- Pidfile: `state/loops/<name>.pid` beside `<name>.log`.
